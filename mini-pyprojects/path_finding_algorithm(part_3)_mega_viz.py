    ...
    def search(x,y):
        ...
        while len(frontier) > 0:          # exit while loop when frontier queue equals zero
            ...
            if (x, y - siz) in path and (x, y - siz) not in visited:  # check the cell down
                ...
                visited.add((x, y - siz))
                print(solution)
            if (x-siz, y - siz) in path and (x-siz, y - siz) not in visited:  # check the cell down
                cell = (x-siz, y - siz)
                solution[cell] = x, y
                #blue.goto(cell)
                #blue.stamp()
                frontier.append(cell)
                visited.add((x-siz, y - siz))
                print(solution)
            if (x+siz, y + siz) in path and (x+siz, y + siz) not in visited:  # check the cell down
                cell = (x+siz, y + siz)
                solution[cell] = x, y
...
def s_point(events):
    ...
    kt=my[lix]
    if s_check==True:
        c.create_oval (x2-3,y2-3,x2+w+3,y2+w+3,fill="deep sky blue",outline="white")
