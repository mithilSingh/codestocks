...
class Circle:
    ...
    def update(self):
        ...
        print("####################################################",self.vy)'''
        #print(self.x, self.y)
        pg.draw.circle(dis, self.color, (self.x, self.y), self.radius, 1 )
            ...
            if i != self:
                ...
                if k < self.radius + i.radius:
                    try:
                        ...
                        '''
                        self.vx = -(i.x - pre_x)*0.2
                        self.vy = -(i.y - pre_y)*0.2
...
while run:
    ...
    pg.draw.circle(dis, mo.color, (mo.x, mo.y), mo.radius, randint(1,1))
    mo.lastUP = (mo.x, mo.y)
    for i in l2:
