...
s_check=True
e_check=True
li=list("+"+(840*" ")+"+" for i in range(550))
li.insert(0,("+")*840)
li.insert(550,("+")*840)
    ...
    def setup_maze(grid):                          # define a function called setup_maze
        ...
        for y in range(len(grid)):                 # read in the grid line by line
            for x in range(len(grid[y])):          # read each cell in the line
                ...
                screen_x = -500 + (x * 5)# move to the x location on the screen staring at -588
                screen_y = 340 - (y * 5) # move to the y location of the screen starting at 288
...
openb.place(x=200,y=650)   
b56=Button(root,text="save",command=saveit)
b56.place(x=100,y=650)
